# @ai-oauth-sdk/svelte example

`createAuth()` wired into a component, with a sign-in that actually completes.

```bash
pnpm install
pnpm --filter @ai-oauth-sdk-example/svelte dev
```
...
redirect URI, then in [`src/App.svelte`](src/App.svelte) swap the descriptor for a
provider id:
