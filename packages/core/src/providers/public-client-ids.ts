 ...
 *
 * **Gemini is the odd one out**: its installed-app clients carry a
 ...
 *
 * Only Gemini. OAuth for installed applications treats these as non-secret:
 * they ship inside a binary anyone can read, and the vendor
 * [documents them as such](https://developers.google.com/identity/protocols/oauth2/native-app).
 * PKCE, not the secret, is what protects the flow, and the token endpoint
