...
An integrator who genuinely has a plaintext internal IDP keeps the documented escape hatch: pass `authorizationUrl` and `tokenUrl` explicitly and they are left alone, exactly as `defineProvider()` leaves them. The difference is that those are then values they typed, not values a remote party chose.
