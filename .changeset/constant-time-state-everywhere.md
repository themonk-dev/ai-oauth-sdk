...
No behaviour change. `timingSafeEqual()` folds the length difference into its accumulator and substitutes `0` past the end of the shorter string, so every pair that compared equal before compares equal now, a missing `state` included.
